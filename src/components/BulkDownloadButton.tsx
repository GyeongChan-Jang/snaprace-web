...
export function BulkDownloadButton({
  ...
  const photosToDownload = isSelectionMode && selectedPhotos.length > 0 
    ? selectedPhotos 
    : photos;
  ...
  const handleBulkDownload = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : "Download failed. Please try selecting fewer photos.");
    } finally {
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            variant={downloadComplete ? "default" : tooManySelected ? "destructive" : "outline"}
            ...
            {isDownloading ? (
              ...
              <>
                ...
                <span className="sm:hidden">
                  {tooManySelected 
                    ? `Too Many (${photoCount})` 
                    : (isSelectionMode && selectedPhotos.length > 0 ? "Selected" : "All") + ` (${photoCount})`}
                </span>
              </>
            )}
          </Button>
